% Supplementary Information for NAR submission (single-column, journal-agnostic).
...
% Numbering: keep plain arabic; the existing \figurename = "Supplementary Fig." renders as "Supplementary Fig. 1, 2, ..."
% so main-text "Supplementary Fig. N" references continue to match.

\title{Supplementary Information for:\\ A shape-constrained regression and wild bootstrap framework for reproducible drug synergy testing}
\author{Amir Asiaee, James P. Long, Samhita Pal, Heather H. Pua, Kevin R. Coombes}
\date{}
...
The SIR software and all analysis code are available at \texttt{https://github.com/AsiaeeLab/isosynergy}.
...
On a sample of 1{,}918 NCI-ALMANAC matrices with valid (non-NA) scores for all four baselines, Bliss and ZIP correlate near-perfectly ($r>0.99$), while Loewe disagrees strongly with all other methods: sign disagreement rates of 38--43\% and top-5\% Jaccard overlaps as low as 0.06 (Supplementary Fig.~\ref{fig:s2_nci_disagreement}).
...
On 120 NCI-ALMANAC matrices subjected to the pseudo-null procedure (sign-flipping df-corrected residuals, $B=200$), the resulting p-values are approximately uniform (Supplementary Fig.~\ref{fig:s3_nci_calibration}; median p-value 0.493), confirming proper calibration on an independent dataset.
...
\section{Supplementary Note 4: per-drug FDR control as a complementary stratification}

The main text controls FDR within each cell line, treating the cell line as the natural screening unit.
A complementary, drug-centric stratification is to control FDR within each drug's family, defined as all matrices containing that drug (across its partner drugs and across cell lines).
This answers a different question, ``for drug $X$, which of its partner combinations are synergistic?'', useful when prioritizing combination partners for a drug already under development.
The two stratifications produce different but coherent answers: drug families overlap (each matrix belongs to two drug families, one per constituent drug), so a matrix can be a discovery in one drug's family but not the other; we report a matrix as per-drug-significant if it clears within-family Benjamini--Hochberg ($\alpha=0.05$) in at least one of its two families.

Applied to the same $B=20{,}000$ p-values (\emph{Adaptive resampling}), the per-drug procedure flags 6{,}396 matrices in total, comparable to the 5{,}260 from the per-cell-line procedure.
Drug-family sizes are large (median 4{,}863; maximum 11{,}741), so the bootstrap-resolution floor $B \ge N/(\alpha R)$ binds for the largest families; the chosen $B=20{,}000$ is at the edge of what those families can resolve, consistent with the modest gain over the per-cell-line tally.

Notably, the same concentration pattern observed across cell lines (18 of 90 cell lines carry all discoveries) recurs across drugs: only 18 of 125 drugs in DrugCombDB have any per-drug FDR-significant matrix (Supplementary Fig.~\ref{fig:per_drug}).
The top contributors are dominated by broadly active agents and DNA-damage/cell-cycle inhibitors (Dinaciclib, Vinorelbine, MK-8776, Geldanamycin, Bortezomib), consistent with classes that have many documented synergistic partners.
Pooling across cell lines does dilute context, however: a drug's synergy may be confined to specific cellular backgrounds (consistent with the per-cell-line localization), so a small per-drug count does not imply absence of synergy for that drug in some cell lines.
We therefore retain the per-cell-line stratification as the primary analysis and present the per-drug results here as a partner-prioritization view.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{\detokenize{per_drug_synergy_partners.pdf}}
  \caption{\textbf{Per-drug FDR-significant matrices on DrugCombDB} (top 20 of 125 drugs; Benjamini--Hochberg within each drug's family, $\alpha=0.05$, $B=20{,}000$).
  Each bar shows the number of FDR-significant drug-pair/cell-line matrices for the indicated drug; the annotation reports hits over family size.
  Discoveries concentrate in 18 of 125 drugs, paralleling the 18-of-90 concentration observed across cell lines.}
  \label{fig:per_drug}
\end{figure}
